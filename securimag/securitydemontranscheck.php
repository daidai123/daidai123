...
//die();
 ...
 if ($_POST["check_demond_post"]=="Submit") {
  ...
  if($valid == true) {
        ...
        $sql = "INSERT INTO `demandpost` (`id`,`title`,`catergory`,`userid`, `countryid`, `stateid`, `cityid`, `countryto`, `stateto`, `cityto`,`pubic`, `replytoemail`, `showemailtype`, `description`, `postdate`, `post_type`,`postlocation`,`ip`, `longtitude`, `latitude`,`view`, `email_check`)  VALUES ";
        $sql .= "(null,'$title','$catergory', $user_id, $country, $state, $city, $countryback, $stateback, $cityback, '$contactpub', '$email', $emailshow, '".$postdescription."', '$postdate', 2,".$iplocation_c.", '".$ip."', ".$locationinfo["longitude"].", ".$locationinfo["latitude"].",0, 'N')";
		echo $sql;
	    ...
	    header("Location: ../postsu.php?pt=2&op=pb&pid=".$fID);
	    die(); 
  } else {
        ...
        <td><table cellpadding="8" cellspacing="8" border="0" width="30%">
			...
			<tr>
			  ...
			  <div style="background:#F9F9F9; border:#999999 solid 1px;">
                 <form method="post" action="securitydemontranscheck.php" enctype="multipart/form-data">
