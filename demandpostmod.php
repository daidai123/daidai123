...
$postid=$_GET["pid"];
CHECK_ID_CORRECT_PAGE('demandpost', 1, $postid, $db);
	...
	<div class="UIFullPage_Container" style="width:860px;">
	   ...
	   <div style=" width:100%;">
		 ...
		 <div style=" margin-top:10px;">
	      ...
	      <form name="postdemondform" id="postdemondform"  enctype="multipart/form-data" action="demondpreview.php" method="post">          
			   <table cellpadding="3" cellspacing="3" border="0" width="100%">
               <tr>
                <td style="width:100%;" colspan="6" nowrap="nowrap">
			     <input type="text" class="inputtext" id="title" style="width:98%; font-size:18px;" maxlength="65" name="title" value="<?php if($_SESSION["demond_add_title"]!="") echo $_SESSION["demond_add_title"]; else echo $title_mod;?>" /><span style="color:#FF0000;">*</span>
              </td>
                 </tr>
		   ...
		   <?php
			...
			?>
			  ...
			  <?php if($_SESSION["demond_add_country"]!=""){ ?>
			  
			    <?php  while ($row = mysql_fetch_array($country_execute_query)) {
			    if($_SESSION["demond_add_country"]==$row["id"]) $selected="selected=\"selected\"";
				else $selected="";
			    ?>
			 ...
			 <?php } ?>
			  ...
			  <option value=""><?php echo PLEASESELECTED; ?></option>
			   <?php if($_SESSION["demond_add_state"]!=""){
			     $state_query="SELECT * FROM `states` where `country`= ".$_SESSION["demond_add_country"]." ORDER BY `id`";
	             $state_execute_query = mysql_query($state_query, $db);
			   ?>
			   <?php  while ($row = mysql_fetch_array($state_execute_query)) {
			    if($_SESSION["demond_add_state"]==$row["id"]) $selected="selected=\"selected\"";
				else $selected="";
			   ?>
			 ...
			 <td id="city_border" style="width:25%;" nowrap="nowrap">
			  ...
			  <?php if($_SESSION["demond_add_city"]!=""){
			    $city_query="SELECT * FROM `city` where `state_id`= ".$_SESSION["demond_add_state"]." ORDER BY `id`";
	           $city_execute_query = mysql_query($city_query, $db); 
			   ?>
			    <?php  while ($row = mysql_fetch_array($city_execute_query)) {
			    if($_SESSION["demond_add_city"]==$row["id"]) $selected="selected=\"selected\"";
				else $selected="";
			  ?>
			  ...
			  <td></td><td><span id="city_check"></span></td>
				...
				<tr>
			  <?php 
				 if($_SESSION["demond_add_contactpub"]!=""){
				  if($_SESSION["demond_add_contactpub"]=="Y"){
			 ...
			 <tr>
               <?php 
				 if($_SESSION["demond_add_emailshow"]!=""){
				  if($_SESSION["demond_add_emailshow"]=="2"){
...
if($_SESSION["demond_add_postdescription"]!="")$oFCKeditor->Value = $_SESSION["demond_add_postdescription"];
