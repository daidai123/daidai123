...
if($actiontype=="mod") {
  ...
  $postid=$_POST["pid"];
  $nextactionpage="supplypostpreview-t.php";
} else{
  $postdate=date("Y-m-d H:i:s");
  $nextactionpage="securimag/securitysupplycheck-cht.php";
}


$_SESSION["trans_supply_add_email"]=$email;
$_SESSION["trans_supply_add_title"]=$title;
$_SESSION["trans_supply_add_country"]=$countryid;
$_SESSION["trans_supply_add_state"]=$stateid;
$_SESSION["trans_supply_add_city"]=$cityid;
$_SESSION["trans_supply_add_countryback"]=$countryback;
$_SESSION["trans_supply_add_stateback"]=$stateback;
$_SESSION["trans_supply_add_cityback"]=$cityback;
$_SESSION["trans_supply_add_catergory"]=$catergory;
$_SESSION["trans_supply_add_contactpub"]=$contactpub;
$_SESSION["trans_supply_add_emailshow"]=$emailshow;
$_SESSION["trans_supply_add_postdescription"]=$postdescription;
$_SESSION["trans_supply_add_backdate"]=$backdate;

$online_offline = "<span style=\"font-size:18; font-weight:bold; color: green;\">".ONLINE."</span>";

if($_POST["confirm"]!=""){
		
		 $sql = "UPDATE `supplypost` SET `title`='".$title."', `catergory` = '".$catergory."',`countryid`= ".$countryid.", `countryback`=".$countryback.",`stateback`=".$stateback.", `cityback`=".$cityback.", `price`='".$price."', `backtime`='".$backdate."', `stateid` = ".$stateid.", `cityid`= ".$cityid.", `pubic`='".$contactpub."', `replytoemail` = '".$email."', `showemailtype`=".$emailshow.", `description` = '".$postdescription."' WHERE`id` = ".$postid."";
		...
		unset($_SESSION["trans_supply_add_email"]);
        unset($_SESSION["trans_supply_add_title"]);
        unset($_SESSION["trans_supply_add_country"]);
        unset($_SESSION["trans_supply_add_state"]);
        unset($_SESSION["trans_supply_add_city"]);
        unset($_SESSION["trans_supply_add_countryback"]);
        unset($_SESSION["trans_supply_add_stateback"]);
        unset($_SESSION["trans_supply_add_cityback"]);
        unset($_SESSION["trans_supply_add_catergory"]);
        unset($_SESSION["trans_supply_add_contactpub"]);
        unset($_SESSION["trans_supply_add_emailshow"]);
		unset($_SESSION["trans_supply_add_postdescription"]);
		unset($_SESSION["trans_supply_add_backdate"]);
		header("Location: postsu.php?pt=2&t=s&op=m&pid=".$postid);
	    die();	
}
...
if($contactpub=='Y'){
   $contactinf_link="contactinfo.php?t=t&keepThis=true&TB_iframe=true&height=450&width=600";
  ...
  <?php } ?>
   ...
   <table cellpadding="4" cellspacing="4" style="width:100%;">
	 ...
	 <?php if($_SESSION["interbringeruserId"]>0){?>
	   ...
	   <a href="javascript:void(0)" style="font-size:18px; color:#0C36E0; font-weight:bold;" onClick="javascript:alert('<?php echo ONLY_OPEN_FOR_RE_USER;?>');"><img src="<?php echo LANGUAGE_SELECT_FOLDER_CHOOSE; ?>/geiwoliuyan.png" /></a>
	   
	   <?php } ?>
	 
	 </td>
	 <td style="text-align:right;"><span style="color:#FF0000; font-weight:bold; font-size:13px;"><?php echo $online_offline;?></span></td>
     </tr>
	 <tr>
	 <td colspan="3">
	 ...
	 <td colspan="1" style="text-align:left;"><?php echo get_city_name($cityid, $db); ?></td>
		 ...
		 <tr>
	 <td colspan="3" nowrap="nowrap"  style=" font-weight:bold;width:100%;">
	  <?php echo BACK_TO; ?>：
	 </td>
	</tr>
	 <tr>
     <td style="width:30px;"></td>
	 <td colspan="1" align="left" style="  width:50px; text-align:left"><?php echo COUNTRY; ?>:</td>
	 <td colspan="1" style="text-align:left;"><?php echo get_country_name($countryback, $db);?></td>
	</tr>
	
	 <tr>
     <td style="width:40px;"></td>
	 <td colspan="1" align="left" style=" width:50px; text-align:left"><?php echo STATE_OR_PROVINCE; ?>:</td>
	 <td colspan="1" style=" text-align:left;"><?php echo get_state_name($stateback, $db);?></td>
	</tr>
	
	 <tr>
     <td style="width:30px;"></td>
	 <td colspan="1" align="left" style=" width:50px; text-align:left"><?php echo CITY; ?>:</td>
	 <td colspan="1" style=" text-align:left;"><?php echo get_city_name($cityback, $db); ?></td>
	</tr>
...
<div style="text-align: center; padding-top:10px; margin-top:5px;">	
	 ...
	 <input type="hidden" id="country" name="country" value="<?php echo $countryid; ?>" />
	 <input type="hidden" id="state" name="state" value="<?php echo $stateid; ?>" />
	 <input type="hidden" id="city" name="city" value="<?php echo $cityid; ?>" />
	 ...
	 <input type="hidden" id="pid" name="pid" value="<?php echo $postid; ?>" />
			...
			<input type="button" value="<?php echo EDIT; ?>" name="edit" onclick="javascript: history.go(-1);" id="edit" style="width:100px; font-weight:bold; height:40px; font-size:16px;background:#EFEFEF; color:#000000;" />
			 <input type="submit" value="<?php echo POST; ?>" style="background:#3B5998; width:100px; height:40px; font-size:16px; font-weight:bold; color:#FFFFFF;" name="confirm"/>
			 </form>
	         </div>
   <?php
