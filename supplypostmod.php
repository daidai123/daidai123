...
$postid=$_GET["pid"];
CHECK_ID_CORRECT_PAGE('supplypost', 1, $postid, $db);
	...
	<div class="UIFullPage_Container" style="width:860px;">
	   ...
	   <div style=" width:100%;">
		 ...
		 <div style=" margin-top:10px;">
	      ...
	      <form name="postsupplyform" id="postsupplyform" enctype="multipart/form-data" action="supplypostpreview.php" method="post">               
			   <table cellpadding="3" cellspacing="3" border="0" width="100%">
               <tr>
                <td style="width:100%;" colspan="6" nowrap="nowrap">
			     <input type="text"  id="title" class="inputtext" style="width:98%; font-size:18px;" name="title" value="<?php if($_SESSION["supply_add_title"]!="") echo $_SESSION["supply_add_title"]; else echo $title;?>" /><span style="color:#FF0000;">*</span>            
              </td>
              </tr>
		   ...
		   <tr>
			  <td colspan="6" nowrap="nowrap" style="font-family:'Times New Roman', Times, serif; font-size:15px;">
			     <?php echo BACK_HOME_TIME; ?>: <input type="text" class="inputtext" id="backdate" name="backdate" onClick="fPopCalendar('backdate')"  readonly="readonly" value="<?php if($_SESSION["supply_add_backdate"]!="") echo $_SESSION["supply_add_backdate"]; else echo $backdate;?>" />
              </td> 
		   </tr>
		   ...
		   <?php
			...
			?>
			  ...
			  <?php if($_SESSION["supply_add_country"]!=""){ ?>
			  
			    <?php  while ($row = mysql_fetch_array($country_execute_query)) {
			    if($_SESSION["supply_add_country"]==$row["id"]) $selected="selected=\"selected\"";
				else $selected="";
			    ?>
			 ...
			 <?php }
			  ...
			  <option value=""><?php echo PLEASESELECTED; ?></option>
			  <?php if($_SESSION["supply_add_state"]!=""){
			     $state_query="SELECT * FROM `states` where `country`= ".$_SESSION["supply_add_country"]." ORDER BY `id`";
	             $state_execute_query = mysql_query($state_query, $db);
			   ?>
			   <?php  while ($row = mysql_fetch_array($state_execute_query)) {
			    if($_SESSION["supply_add_state"]==$row["id"]) $selected="selected=\"selected\"";
				else $selected="";
			   ?>
			 ...
			 <td id="city_border" style="width:25%;" nowrap="nowrap">
			  ...
			  <option value=""><?php echo PLEASESELECTED; ?></option>
			  <?php if($_SESSION["supply_add_city"]!=""){
			    $city_query="SELECT * FROM `city` where `state_id`= ".$_SESSION["supply_add_state"]." ORDER BY `id`";
	           $city_execute_query = mysql_query($city_query, $db); 
			   ?>
			    <?php  while ($row = mysql_fetch_array($city_execute_query)) {
			    if($_SESSION["supply_add_city"]==$row["id"]) $selected="selected=\"selected\"";
				else $selected="";
			  ?>
			  ...
			  <option value=""><?php echo PLEASESELECTED; ?></option>
			  <?php if($_SESSION["supply_add_countryback"]!=""){ ?>
			  
			    <?php  while ($row = mysql_fetch_array($country_execute_query)) {
			    if($_SESSION["supply_add_countryback"]==$row["id"]) $selected="selected=\"selected\"";
				else $selected="";
			    ?>
			  <option value="<?php echo $row["id"];?>" <?php echo $selected;?>><?php echo $row["country"]; ?></option>
			  <?php } ?>
			 ...
			 <?php }
			  ...
			  ?>
			   ...
			   <option value=""><?php echo PLEASESELECTED; ?></option>
			   <?php if($_SESSION["supply_add_stateback"]!=""){
			     $state_query="SELECT * FROM `states` where `country`= ".$_SESSION["supply_add_countryback"]." ORDER BY `id`";
	             $state_execute_query = mysql_query($state_query, $db);
			   ?>
			   <?php  while ($row = mysql_fetch_array($state_execute_query)) {
			    if($_SESSION["supply_add_stateback"]==$row["id"]) $selected="selected=\"selected\"";
				else $selected="";
			   ?>
			  <option value="<?php echo $row["id"];?>" <?php echo $selected;?>><?php echo $row["state_name"]; ?></option>
			  <?php   } ?>
			   ...
			   <?php } ?>
			    ...
			    <option value=""><?php echo PLEASESELECTED; ?></option>
			    <?php if($_SESSION["supply_add_cityback"]!=""){
			    $city_query="SELECT * FROM `city` where `state_id`= ".$_SESSION["supply_add_stateback"]." ORDER BY `id`";
	            $city_execute_query = mysql_query($city_query, $db); 
			    ?>
			    <?php  while ($row = mysql_fetch_array($city_execute_query)) {
			    if($_SESSION["supply_add_cityback"]==$row["id"]) $selected="selected=\"selected\"";
				else $selected="";
			  ?>
			  <option value="<?php echo $row["id"];?>" <?php echo $selected;?>><?php echo $row["name"]; ?></option>
			  <?php   } ?>
				...
				<tr>
				<?php 
				 if($_SESSION["supply_add_contactpub"]!=""){
				  if($_SESSION["supply_add_contactpub"]=="Y"){
			 ...
			 <tr>
			 <?php 
				 if($_SESSION["supply_add_emailshow"]!=""){
				  if($_SESSION["supply_add_emailshow"]=="2"){
              ...
              <td  colspan="6" style="font-family:'Times New Roman', Times, serif; font-size:15px;" nowrap="nowrap">
				...
				<tr>
                 ...
                 <?php
                  ...
                  if($_SESSION["supply_add_postdescription"]!="")$oFCKeditor->Value = $_SESSION["supply_add_postdescription"];
